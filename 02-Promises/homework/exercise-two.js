...
function problemA () {
 ...
 // );
  ...
  // promise version
  Promise.all([ promisifiedReadFile('poem-one/stanza-01.txt') ,
   promisifiedReadFile('poem-one/stanza-02.txt') ]).then( (arraySuccess) => {
    // arraySuccess.forEach(el => blue(el))
    blue(arraySuccess[0])
    blue(arraySuccess[1])
    console.log('done')
   })
...
function problemC () {
 ...
 // );
  ...
  // promise version
  for (let i = 1, caja =promisifiedReadFile(filenames[0]); 1 <= filenames.length; i++){
